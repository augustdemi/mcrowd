...
class GeometricMap(Map):
    ...
    def get_cropped_maps_from_scene_map_batch(cls, maps, scene_pts, radius, compute_local_homo=False):
        ...
        # all_traj = torch.cat([torch.tensor(scene_pts), fut_traj * data['traj_scale']], dim=1)
        # all_pixel = maps[0].to_map_points(all_traj)
        # all_pixel = radius[0] + np.round(all_pixel).astype(int)

        '''
        plt.imshow(padded_map[0].transpose(2,1,0))
        plt.imshow(maps[0].data.transpose(2,1,0))
        plt.scatter(all_pixel[:4, 0], all_pixel[:4, 1], s=1, c='y')
        plt.scatter(all_pixel[4:, 0], all_pixel[4:, 1], s=1, c='b')
        plt.show()
        '''
        local_maps = []
        local_homos = []
        ...
        for agent_idx in range(len(padded_map)):
            ...
            # for i in range(3):
            #     if np.unique(expanded_obs_img[i]).sum() > 1:
            #         print(np.unique(expanded_obs_img[i]))
            #         print('--------------------')
            ...
            '''
            plt.imshow(local_map.transpose(2,1,0))
            plt.imshow(expanded_obs_img.transpose(2,1,0))
            plt.imshow(expanded_obs_img)
            plt.scatter(all_pixel[:4, 1], all_pixel[:4, 0], s=1, c='b')
            plt.scatter(all_pixel[4:, 1], all_pixel[4:, 0], s=1, c='y')
            '''
            ...
            if compute_local_homo:
                ...
                '''
                plt.imshow(expanded_obs_img)
                plt.scatter(fake_pixel[:, 1], fake_pixel[:, 0], s=1, c='b')
                '''
                ...
                plt.scatter(all_pixel_local[:4, 1], all_pixel_local[:4, 0], s=1, c='b')
                plt.scatter(all_pixel_local[4:, 1], all_pixel_local[4:, 0], s=1, c='r')
                plt.show()
