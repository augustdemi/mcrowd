...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        # data = data1[data1['a'] < 10]
            ...
            print(path)
            cum_start_idx = [0] + np.cumsum(num_peds_in_seq[keep_len:]).tolist() # num_peds_in_seq = 각 slide(16개 frames)별로 고려된 agent수.따라서 len(num_peds_in_seq) = slide 수 = 2692 = self.num_seq
            aa = np.array([
                (start, end)
                for start, end in zip(cum_start_idx, cum_start_idx[1:])
            ])
            print('total num agent: ', aa[-1][1])
            print('avg num agent in one seq: ', np.round((aa[:, 1] - aa[:, 0]).mean(), 2))
            print('min num agent in one seq: ', np.round((aa[:, 1] - aa[:, 0]).min(), 2))
            print('max num agent in one seq: ', np.round((aa[:, 1] - aa[:, 0]).max(), 2))

            # ss = np.concatenate(seq_list[keep_len:])
            # dist = np.sqrt(((ss[:, :2, 1:] - ss[:, :2, :-1])**2).sum(1))
            # print('avg dist in one seq: ', np.round(().max(), 2))
            print('------------------------------')
            keep_len = len(num_peds_in_seq)
    ...
    def __getitem__(self, index):
        ...
        for _ in range(end-start):
            local_maps.append(np.expand_dims(np.eye(256),0))
        local_ics = np.zeros((end-start,20,2))
        local_homos = np.zeros((end-start,3,3))
