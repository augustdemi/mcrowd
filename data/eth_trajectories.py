...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        local_ics = []
        if 's001' in self.map_file_name[index]:
            resized_map = np.zeros((345 + 15, 432 + 48))
            # resized_map[10:345+10, 30:432+30] = resize(global_map, (3 * global_map.shape[0] // 5, 3 * global_map.shape[1] // 5))
            inv_h_t = np.array([[-5.88012482e-01,  4.75745709e+01, -2.60897585e-04],
                   [-4.11127194e+01, -2.31511663e-01, -1.48289892e-03],
                   [ 5.65486412e+02, -5.13153507e-01,  1.02041521e+00]])
            ...
            for idx in range(start, end):
                ...
                all_pixel /= np.expand_dims(all_pixel[:, 2], 1)
                all_pixel_resized = np.round(3*all_pixel / 5)[:,:2] + (10, 30)
                plt.scatter(all_pixel_resized[:8,1], all_pixel_resized[:8,0], s=1, c='b')
                plt.scatter(all_pixel_resized[8:,1], all_pixel_resized[8:,0], s=1, c='r')

                # all_pixel_resized = (all_pixel_resized - (10,30)) * 5 / 3
                # back_wc = np.matmul(np.concatenate([all_pixel_resized, np.ones((len(all_pixel_resized), 1))], axis=1), np.linalg.pinv(inv_h_t))
                # back_wc /= np.expand_dims(back_wc[:, 2], 1)
                # back_wc = back_wc[:,:2]
                # np.sqrt(((back_wc - all_traj)**2).sum(1)).max()

                local_ics.append(all_pixel_resized)

                # dists = []
                # for idx in range(3400,4400):
                #     all_traj = torch.cat([self.obs_traj[idx, :2], self.pred_traj[idx, :2]], dim=1). \
                #         transpose(1, 0).detach().cpu().numpy()
                #     dists.append(np.sqrt(((all_traj[1:] - all_traj[:-1])**2).sum(1)).mean())
                # dists = np.array(dists)
                # dist_idx = np.where((dists < 0.5) &(dists>0.4))[0]
        else:
            global_map = imageio.imread(self.map_file_name[index])
            inv_h_t = self.inv_h_t[index]
            if global_map.shape[0] == 480:
                resized_map = np.ones((384, 480))
                resized_map[:360, :480] = resize(global_map, (3 * global_map.shape[0] // 4, 3 * global_map.shape[1] // 4))
            else:
                resized_map = resize(global_map, (2 * global_map.shape[0] // 3, 2 * global_map.shape[1] // 3))

            plt.imshow(resized_map)
            for idx in range(500,700):
                all_traj = torch.cat([self.obs_traj[idx, :2], self.pred_traj[idx, :2]], dim=1).\
                    transpose(1, 0).detach().cpu().numpy()
                all_pixel = np.matmul(np.concatenate([all_traj, np.ones((len(all_traj), 1))], axis=1), inv_h_t)
                all_pixel /= np.expand_dims(all_pixel[:, 2], 1)
                all_pixel_resized = np.round(2*all_pixel / 3)[:,:2] + (-10, 2)
                plt.scatter(all_pixel_resized[:8,1], all_pixel_resized[:8,0], s=1, c='b')
                plt.scatter(all_pixel_resized[8:,1], all_pixel_resized[8:,0], s=1, c='r')

                if 'zara03':
                    all_pixel_resized += (-10, 2)


                #eth = 256, zara1 =384 = hotel, students003: 470
                if global_map.shape[0] == 480:
                    all_pixel_resized = np.round(3 * all_pixel / 4)[:, :2]
                else:
                    all_pixel_resized = np.round(2*all_pixel / 3)[:,:2]

                # plt.imshow(resized_map)
                # plt.scatter(all_pixel_resized[:8,1], all_pixel_resized[:8,0], s=1, c='b')
                # plt.scatter(all_pixel_resized[8:,1], all_pixel_resized[8:,0], s=1, c='r')
                #
                # all_pixel_resized = all_pixel_resized[:,:2] * 4 / 3
                # back_wc = np.matmul(np.concatenate([all_pixel_resized, np.ones((len(all_pixel_resized), 1))], axis=1), np.linalg.pinv(inv_h_t))
                # back_wc /= np.expand_dims(back_wc[:, 2], 1)
                # back_wc = back_wc[:,:2]
                # np.sqrt(((back_wc - all_traj)**2).sum(1)).max()

                # all_pixel_resized = (all_pixel_resized[:,:2] -(-10,2))* 3 / 2
                # back_wc = np.matmul(np.concatenate([all_pixel_resized, np.ones((len(all_pixel_resized), 1))], axis=1), np.linalg.pinv(inv_h_t))
                # back_wc /= np.expand_dims(back_wc[:, 2], 1)
                # back_wc = back_wc[:,:2]
                # np.sqrt(((back_wc - all_traj)**2).sum(1)).max()
        ...
        local_maps = np.array([resized_map] * (end - start))
        local_homos = np.array([inv_h_t] * (end - start))
        ...
        out = [
            ...
            local_maps, local_ics, local_homos
        ]
        return out
