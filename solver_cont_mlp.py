...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        self.name = '%s_pred_len_%s_zS_%s_embedding_dim_%s_enc_h_dim_%s_dec_h_dim_%s_mlp_dim_%s_pool_dim_%s_lr_%s' % \
                    (args.dataset_name, args.pred_len, args.zS_dim, args.embedding_dim, args.encoder_h_dim, args.decoder_h_dim, args.mlp_dim, args.pool_dim, args.lr_VAE)
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter + 1):
            ...
            # (dist_fc_inputMx, muPrivateMx, stdPrivateMx, logvarPrivateMx, muSharedMx, stdSharedMx, logvarSharedMx) \
            #     = self.encoderMx(obs_traj_rel, seq_start_end)
            # (dist_fc_inputMy, muPrivateMy, stdPrivateMy, logvarPrivateMy, muSharedMy, stdSharedMy, logvarSharedMy) \
            #     = self.encoderMx(fut_traj_rel, seq_start_end, coditioned_h=dist_fc_inputMx)
            ...
            # zPrivateMx = sample_gaussian(self.use_cuda, muPrivateMx, stdPrivateMx)
            # zPrivateMy = sample_gaussian(self.use_cuda, muPrivateMy, stdPrivateMy)

            # zSharedMy = sample_gaussian(self.device, muSharedMy, stdSharedMy)
            zSharedMy = sample_gaussian(self.device, torch.zeros_like(muSharedMy), torch.ones_like(stdSharedMy))
            # decoder_c = torch.zeros(self.num_layers, batch, self.decoder_h_dim).to(self.device)
            # decoder_h = torch.cat([dist_fc_inputMx, zSharedMy], dim=1).unsqueeze(0)
            ...
            loss_kl = kl_two_gaussian(muSharedMy, stdSharedMy**2, muSharedMx, stdSharedMx**2).div(batch)
        ...
        with torch.no_grad():
            ...
            for batch in data_loader:
                ...
                if loss:
                    ...
                    loss_kl += kl_two_gaussian(muSharedMy, stdSharedMy ** 2, muSharedMx, stdSharedMx ** 2).div(batch_size)
                    vae_loss += (loss_recon + loss_kl)
