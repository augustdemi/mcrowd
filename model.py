...
class EncoderX(nn.Module):
    ...
    def forward(self, obs_traj, seq_start_end, local_map, train=False):
        ...
        return hx, all_map_feat, prior_stat[:,:self.zS_dim], prior_stat[:,self.zS_dim:]
...
class EncoderY(nn.Module):
    ...
    def forward(self, map_feat, last_obs_traj, fut_vel, seq_start_end, local_map, train=False):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
        Output:
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_context= nn.Linear(dec_h_dim + context_dim + map_feat_dim, dec_h_dim)

    def forward(self, map_feat, seq_start_end, last_obs_st, last_pos, enc_h_feat, z, sg, sg_update_idx, fut_vel_st=None, train=False):
        """
        Inputs:
        ...
        for i in range(self.seq_len):
            ...
            if self.context_dim > 0:
                ...
                decoder_h = self.mlp_context(torch.cat([decoder_h, context, F.normalize(map_feat)], dim=1))  # mlp : 1152 -> 1024 -> 128
