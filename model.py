...
class MapEncoder(nn.Module):
    ...
    def forward(self, last_obs_vel, map, train=False):
        """
        Inputs:
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.map_encoder = MapEncoder(map_mlp_dim, map_feat_dim, dropout_mlp)
    ...
    def forward(self, obs_traj, seq_start_end, local_map, train=False):
        ...
        map_feat = self.map_encoder(obs_traj[-1,:,2:4], local_map, train=True)


        # fc1 only to traj
        dist_fc_input = self.fc1(final_encoder_h.view(-1, self.enc_h_dim))
        stats = self.fc2(torch.cat((dist_fc_input, map_feat), dim=-1)) # 64(32 without attn) to z dim

        # fc1 with map
        # dist_fc_input = self.fc1(torch.cat((final_encoder_h.view(-1, self.enc_h_dim), map_feat), dim=-1))
...
class EncoderY(nn.Module):
    ...
    def forward(self, last_obs_traj, fut_vel, seq_start_end, obs_enc_feat, train=False):
        ...
        # final distribution
        dist_fc_input = self.fc1(final_encoder_h.reshape(-1, 4 * self.enc_h_dim))
