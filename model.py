...
class Decoder(nn.Module):
    ...
    def forward(self, seq_start_end, last_obs_st, last_pos, enc_h_feat, z, sg, sg_update_idx, fut_vel_st=None, train=False):
        ...
        curr_pos = pred_vel * self.dt + last_pos
        context = self.pool_net(enc_h_feat, seq_start_end, curr_pos)  # batchsize, 1024
        # enc_context = self.mlp_context(torch.cat([enc_h_feat, context], dim=1))
...
class PoolHiddenNet(nn.Module):
    ...
    def forward(self, h_states, seq_start_end, end_pos):
        ...
        for _, (start, end) in enumerate(seq_start_end):
            ...
            mlp_h_input = torch.cat([curr_rel_pos, curr_hidden_1], dim=1) #(repeated data, 64+128)
