...
class GMM2D(td.Distribution):
    ...
    def __init__(self, log_pis, mus, log_sigmas, corrs):
        ...
        self.components = log_pis.shape[-1] # 25 for the last a_dist
        self.dimensions = 2
        self.device = log_pis.device

        log_pis = torch.clamp(log_pis, min=-1e5) #[1,256,12,25] for the last a_dist., [20, 1] for test
        self.log_pis = log_pis - torch.logsumexp(log_pis, dim=-1, keepdim=True)  # [..., N] # (6400, 1) : N=1 component
        self.mus = self.reshape_to_components(mus)         # [..., N, 2] # (6400(256*25), 1, 2), or [1, 256, 12, 25, 2] for the last dist. [20, 1, 2] fro test time
        self.log_sigmas = self.reshape_to_components(log_sigmas)  # [..., N, 2]# (6400(256*25), 1, 2)
        ...
        self.corrs = corrs  # [..., N] #6400, 1
        # L = [6400, 1, 2, 2]
        self.L = torch.stack([torch.stack([self.sigmas[..., 0], torch.zeros_like(self.log_pis)], dim=-1), # [6400, 1, 2]
    ...
    def log_prob(self, value):
        ...
        """
        # x: [..., 2]
        value = torch.unsqueeze(value, dim=-2)       # [..., 1, 2] # 256,12,2 --> [256, 12, 1, 2]
        dx = value - self.mus                       # [..., N, 2]

        exp_nominator = ((torch.sum((dx/self.sigmas)**2, dim=-1)  # first and second term of exp nominator (sigma = 1, 256, 12, 25, 2)
                          - 2*self.corrs*torch.prod(dx, dim=-1)/torch.prod(self.sigmas, dim=-1)))    # [..., N]  [1, 256, 12, 25]
        ...
        return torch.logsumexp(self.log_pis + component_log_p, dim=-1) # log_pis = component_log_p =[1, 256, 12, 25]
