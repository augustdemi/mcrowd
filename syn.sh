gpu=3
run_id=22
desc='normal,withTF,min_ped=1,temp=1.99,cropped,mean0,stard,goal_thres=0.1,nofc'
dataset_name='s6'
kl_weight=100
z_S=64
dropout_mlp=0.1
...
CUDA_VISIBLE_DEVICES=${gpu} \
	...
	--print_iter 10 --map_size ${map_size} --map_trainable ${map_trainable} \
	...
	> ./slog/${dataset_name}.${run_id}.pred_len${pred_len}.z_S${z_S}.drop_mlp${dropout_mlp}.drop_rnn${dropout_rnn}.encoder_h_dim${encoder_h_dim}.dec_h_dim${decoder_h_dim}.mlp_dim${mlp_dim}.lr_VAE${lr_VAE}.bs${batch_size}.kl_w${kl_weight}.attn${attention}.map_size${map_size}.map_tr${map_trainable}.txt &
