...
class EncoderX(nn.Module):
    ...
    def forward(self, src, src_mask):
        logit_token = Variable(torch.FloatTensor(np.random.rand(src.shape[0], 1, self.d_model)))
...
class EncoderY(nn.Module):
    ...
    def forward(self, src_trg, src_trg_mask):
        logit_token = Variable(torch.FloatTensor(np.random.rand(src_trg.shape[0], 1, self.d_model)))
