...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        self.name = '%s_bs_%s_enc_block_%s_fcomb_block_%s_wD_%s_lr_%s_lg_klw_%s_fb_%s_anneal_e_%s_load_e_%s_k_%s' % \
                    (args.dataset_name, args.batch_size, args.no_convs_per_block, args.no_convs_fcomb, args.w_dim,
                     args.lr_VAE,
                     args.lg_kl_weight, args.fb, args.anneal_epoch, args.load_e, args.k_fold)
        ...
        if args.load_e > 0:
            lg_cvae_path = 'large.lg.ae_bs_%s_enc_block_%s_fcomb_block_%s_wD_10_lr_0.0001_k_%s_run_%s' % (
                args.batch_size, args.no_convs_per_block, args.no_convs_fcomb, args.k_fold, args.run_id)
        ...
        with torch.no_grad():
            ...
            for batch in data_loader:
                ...
                lg_fde.append(torch.sqrt(((torch.stack(pred_lg_wc20)
                                           - fut_traj[:, :2, -1].unsqueeze(0).repeat((num_p, 1, 1))) ** 2).sum(
                    -1)))  # 20, 3, 4, 2
