...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        self.sg_unet = _CustomDataParallel(self.sg_unet)
        ...
        if self.ckpt_load_iter != self.max_iter:
            ...
            tr_dset = TrajectoryDataset(args.dataset_dir, data_split='train', device=self.device)
            self.train_loader = DataLoader(dataset=tr_dset, batch_size=args.batch_size,
                                              shuffle=True, num_workers=args.loader_num_workers)
            val_dset = TrajectoryDataset(args.dataset_dir, data_split='val', device=self.device)
            self.val_loader = DataLoader(dataset=val_dset, batch_size=args.batch_size,
                                              shuffle=True, num_workers=args.loader_num_workers)
            # _, self.train_loader = data_loader(self.args, args.dataset_dir, 'test2', shuffle=True)
            # print("Initializing val dataset")
            # _, self.val_loader = data_loader(self.args, args.dataset_dir, 'test2', shuffle=True)
    ...
    def make_heatmap(self, local_ic, local_map, aug=False):
        ...
        return all_heatmaps[:, :2], all_heatmaps[:, 2:], all_heatmaps[:,-1].unsqueeze(1)
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter + 1):
            ...
            if epoch > 10 and (iteration % (iter_per_epoch * 10) == 0):
        ...
        with torch.no_grad():
            ...
            for batch in data_loader:
                ...
                for _ in range(5):
                    ...
                    pred_sg_heat = self.sg_unet.forward(torch.cat([obs_heat_map, lg_heat_map], dim=1))
                ...
                lg_fde.append(torch.sqrt(((torch.stack(pred_lg_wcs)
                                           - fut_traj[:,:2, -1].unsqueeze(0).repeat((5,1,1)))**2).sum(-1))) # 20, 3, 4, 2
                lg_fde2.append(torch.sqrt(((torch.stack(pred_sg_wcs)[:,:,-1]
                                           - fut_traj[:,:2, -1].unsqueeze(0).repeat((5,1,1)))**2).sum(-1)))
                sg_ade.append(torch.sqrt(((torch.stack(pred_sg_wcs) # #k, bs, #sg, x-y,
                                           - fut_traj[:, :2, list(self.sg_idx)].permute(0, 2, 1).unsqueeze(0).repeat(
                    (5, 1, 1, 1))) ** 2).sum(-1)).sum(2))
    ...
    def viz_init(self):
        ...
        self.viz.close(env=self.name, win=self.win_id['sg_recon'])
        self.viz.close(env=self.name, win=self.win_id['test_sg_recon'])
        self.viz.close(env=self.name, win=self.win_id['sg_ade_min'])
        self.viz.close(env=self.name, win=self.win_id['sg_ade_avg'])
        self.viz.close(env=self.name, win=self.win_id['sg_ade_std'])
    ...
    def visualize_line(self):
        ...
        self.viz.line(
            X=iters, Y=sg_ade_std, env=self.name,
        ...
        self.viz.line(
            X=iters, Y=sg_ade_avg, env=self.name,
        ...
        self.viz.line(
            X=iters, Y=sg_ade_min, env=self.name,
        ...
        self.viz.line(
            X=iters, Y=sg_recon, env=self.name,
        ...
        self.viz.line(
            X=iters, Y=test_sg_recon, env=self.name,
    ...
    def save_checkpoint(self, iteration):
        ...
        torch.save(self.sg_unet.module.state_dict(), sg_unet_path)
    ...
    def load_checkpoint(self):
        ...
        self.sg_unet.load_state_dict(torch.load(sg_unet_path, map_location=self.device))
