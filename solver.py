...
class Solver(object):
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter + 1):
            ...
            # pred_fut_traj = integrate_samples(vel_pred_prior * self.scale, obs_traj[-1, :, :2], dt=self.dt)

            ll_tf_post = ((vel_pred_post - fut_vel_st)**2).sum().div(batch_size)
            ll_prior = ((vel_pred_prior - fut_vel_st)**2).sum().div(batch_size)
            ...
            if self.w_coll > 0:
                pred_fut_traj = integrate_samples(vel_pred_prior * self.scale, obs_traj[-1, :, :2],
                                                  dt=self.dt)

                pred_fut_traj_post = integrate_samples(vel_pred_post * self.scale,
                                                       obs_traj[-1, :, :2],
                                                       dt=self.dt)
    ...
    def evaluate_dist(self, data_loader, loss=False):
        ...
        with torch.no_grad():
            ...
            while not data_loader.is_epoch_end():
                ...
                ade, fde = [], []
                for vel_prior in vel_pred_prior20:
                    pred_fut_traj = integrate_samples(vel_prior * self.scale, obs_traj[-1, :, :2], dt=self.dt)
