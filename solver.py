...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        self.name = '%s_enc_block_%s_fcomb_block_%s_wD_%s_lr_%s_lg_klw_%s_fb_%s_anneal_e_%s_load_e_%s' % \
                    (args.dataset_name, args.no_convs_per_block, args.no_convs_fcomb, args.w_dim, args.lr_VAE,
                     args.lg_kl_weight, args.fb, args.anneal_epoch, args.load_e)
        ...
        if self.ckpt_load_iter == 0 or args.dataset_name == 'all':  # create a new model
            ...
            if args.load_e > 0:
                lg_cvae_path = 'large.lg.ae_enc_block_%s_fcomb_block_%s_wD_10_lr_0.0001_run_%s' % (
                args.no_convs_per_block, args.no_convs_fcomb, args.run_id)
                lg_cvae_path = os.path.join('ckpts', lg_cvae_path, 'iter_6_lg_cvae.pt')
                ...
                self.lg_cvae = ProbabilisticUnet(input_channels=3, num_classes=1, num_filters=num_filters,
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter + 1):
            ...
            obs_heat_map, lg_heat_map = self.make_heatmap(sampled_local_ic, sampled_local_map, sampled_congest_map,
                                                          aug=True)
            ...
            focal_loss = (self.alpha * lg_heat_map * torch.log(recon_lg_heat + self.eps) * (
                        (1 - recon_lg_heat) ** self.gamma) \
            ...
            if epoch > 90 and (iteration % (iter_per_epoch * 50) == 0) and (epoch < 501):
    ...
    def evaluate_dist(self, data_loader, num_p=20, loss=False):
        ...
        with torch.no_grad():
            ...
            for batch in data_loader:
                ...
                obs_heat_map, lg_heat_map = self.make_heatmap(local_ic, local_map, congest_map)
                ...
                lg_fde.append(torch.sqrt(((torch.stack(pred_lg_wc20)
                                           - fut_traj[-1, :, :2].unsqueeze(0).repeat((num_p, 1, 1))) ** 2).sum(
                    -1)))  # 20, 3, 4, 2
