...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        self.name = '%s_zD%s_dr_mlp%s_dr_rnn%s_enc_hD%s_dec_hD%s_fcD%s_map_featD%s_map_fcD%s_lr%s_klw%s_priorllw%s_wD%s_lgklw%s' \
                    '_fcomb%s_a%s_r%s_fb%s_anneal%s_pretr%s' % \
                    (args.dataset_name, args.zS_dim, args.dropout_mlp, args.dropout_rnn, args.encoder_h_dim,
                     ...
                     args.ll_prior_w, args.w_dim, args.lg_kl_weight, args.no_convs_fcomb, args.alpha, args.gamma, args.fb, args.anneal_epoch, args.load_e
                     )
        ...
        if self.ckpt_load_iter == 0 or args.dataset_name =='all':  # create a new model
            ...
            if args.load_e > 0:
                lg_cvae_path = 'lgcvae_enc_block_1_fcomb_block_%s_wD_%s_lr_%s_a_%s_r_%s_run_%s' % \
                               (
...
###########
            ...
            lg_recon_loss = - (self.alpha * lg_heat_map * torch.log(recon_lg_heat + self.eps) * ((1 - recon_lg_heat) ** self.gamma) \
            ...
            lg_elbo = -lg_recon_loss - lg_kl_weight * lg_kl
            ...
            sg_recon_loss = - (self.alpha * sg_heat_map * torch.log(recon_sg_heat + self.eps) * ((1 - recon_sg_heat) ** self.gamma) \
            ...
            loss_kl = torch.clamp(loss_kl, min=0.07).sum().div(batch_size)
    ...
    def evaluate_dist(self, data_loader, lg_num=4, traj_num=5, loss=False):
