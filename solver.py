...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        self.name = '%s_lr_%s_num_sg_%s' % \
                    (args.dataset_name, args.lr_VAE, args.num_sg)
        ...
        lg_cvae_path = 'large.lgcvae_enc_block_1_fcomb_block_2_wD_10_lr_0.0001_lg_klw_1.0_fb_6.0_anneal_e_10_load_e_3_k_0_run_11'
        lg_cvae_path = os.path.join('ckpts', lg_cvae_path, 'iter_500_lg_cvae.pt')
        if self.device == 'cuda':
        ...
        if self.ckpt_load_iter == 0 or args.dataset_name =='all':  # create a new model
            ...
            self.sg_unet = Unet(input_channels=4, num_classes=len(self.sg_idx), num_filters=num_filters,
    ...
    def make_heatmap(self, local_ic, local_map, congest_map, aug=False):
        ...
        return all_heatmaps[:, :3], all_heatmaps[:, 3:], all_heatmaps[:,-1].unsqueeze(1)
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter + 1):
            ...
            (obs_traj, fut_traj, _, _, seq_start_end,
             ...
             local_map, local_ic, local_homo, congest_map) = next(iterator)
            ...
            for s, e in seq_start_end:
                ...
                sampled_congest_map.append(congest_map[rng[:2]])

            sampled_local_ic = np.concatenate(sampled_local_ic)
            sampled_local_map = np.concatenate(sampled_local_map)
            sampled_congest_map = np.concatenate(sampled_congest_map)

            batch_size = sampled_local_map.shape[0]

            obs_heat_map, sg_heat_map, lg_heat_map = self.make_heatmap(sampled_local_ic, sampled_local_map, sampled_congest_map, aug=True)
            ...
            if epoch > 40 and (iteration % (iter_per_epoch * 10) == 0):
    ...
    def evaluate_dist(self, data_loader, loss=False):
        ...
        with torch.no_grad():
            ...
            for batch in data_loader:
                ...
                (obs_traj, fut_traj, _, _, seq_start_end,
                 obs_frames, pred_frames, map_path, inv_h_t,
                 local_map, local_ic, local_homo, congest_map) = batch
                batch_size = obs_traj.size(1)
                total_traj += fut_traj.size(1)

                obs_heat_map, sg_heat_map, lg_heat_map = self.make_heatmap(local_ic, local_map, congest_map)
