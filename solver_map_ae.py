    ...
    def preprocess_map(self, local_map, aug=False):
        ...
        for i in range(len(local_map)):
            ...
            if map_size < down_size:
                tmp = np.full((down_size, down_size), 3)
                tmp[half-map_size//2:half+map_size//2, half-map_size//2:half+map_size//2] = local_map[i][0]
                env.append(tmp/5)
            else:
                env.append(cv2.resize(local_map[i][0], dsize=(down_size, down_size))/5)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for abatch in self.val_loader:
                ...
                batch_size = obs_traj.sself.preprocess_mapize(1)
