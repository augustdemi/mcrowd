...
class Unet(nn.Module):
    ...
    def __init__(self, input_channels, num_classes, num_filters, latent_loc=3, padding=True):
    ...
    def down_up1_forward(self, x):
        ...
        for i, up in enumerate(self.upsampling_path[:self.latent_loc]):
            x = up(x, self.blocks[-i - 1])

        return x

    def up2_forward(self, x):

        for i, up in enumerate(self.upsampling_path[self.latent_loc:]):
            x = up(x, self.blocks[-(i + self.latent_loc) - 1])
