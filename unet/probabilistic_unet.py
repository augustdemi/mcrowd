...
class ProbabilisticUnet(nn.Module):
    ...
    def forward(self, past_heatmap, past_state, fut_heatmap, training=True):
        ...
        # latent dist
        if fut_heatmap is not None:
            self.posterior_latent_space = self.posterior.forward(past_heatmap, past_state, fut_heatmap, training=training)
            self.prior_latent_space = self.prior.forward(self.unet_enc_feat, past_state, training=training)
        else:
            self.prior_latent_space = self.prior.forward(self.unet_enc_feat, past_state, training=training)
