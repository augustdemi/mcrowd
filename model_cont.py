...
class Encoder(nn.Module):
    ...
    def forward(self, rel_traj, seq_start_end, coditioned_h=None):
        ...
        # final distribution
        stats = self.fc(dist_fc_input)

        muShared = stats[:, :self.zS_dim]
        logvarShared = stats[:, self.zS_dim:]
        stdShared = torch.sqrt(torch.exp(logvarShared))

        return dist_fc_input, muShared, stdShared
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        pool_every_timestep=True, dropout=0.0, pool_dim=1024,
        activation='relu', batch_norm=False, pooling_type='attn'
    ):
        ...
        if pool_every_timestep:
            if pooling_type == 'pool':
                ...
                input_dim = h_dim*2
            mlp_dims = [input_dim, mlp_dim, h_dim]
            self.mlp = make_mlp(
                mlp_dims,
                activation=activation,
                batch_norm=batch_norm,
